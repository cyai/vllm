...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: VllmConfig, prefix: str, layer_idx: int):
        ...
        self.ffn = nn.Sequential(
            ColumnParallelLinear(vllm_config.hidden_size, vllm_config.ffn_size),
            RowParallelLinear(vllm_config.ffn_size, vllm_config.hidden_size),
        )
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: VllmConfig, prefix: str):
        ...
        self.layers = nn.ModuleList(
            ...
            [
                Qwen2DecoderLayer(config, prefix, layer_idx)
                for layer_idx in range(config.model_config.hf_config.num_hidden_layers)
            ]
        )
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, prefix: str = ""):
        super().__init__(config)
        self.model = Qwen2Model(config, prefix=f"{prefix}.model")
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        Args:
            input_ids (`torch.Tensor`): Flattened input token IDs of shape `(total_tokens,)`.
            positions (`torch.Tensor`): Flattened positional indices of shape `(total_tokens,)`.
            kv_caches (`List[torch.Tensor]`, optional): Cached key-value tensors for faster inference.
            attn_metadata (`AttentionMetadata`, optional): Metadata for attention calculations.
    ...
    def load_weights(self, checkpoint_path: str):
        ...
        # Map Hugging Face weights to vLLM model's structure
